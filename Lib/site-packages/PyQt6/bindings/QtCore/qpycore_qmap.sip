...
%MappedType QMultiMap<_TYPE1_, _TYPE2_>
        /TypeHintIn="Dict[_TYPE1_, Sequence[_TYPE2_]]", TypeHintOut="Dict[_TYPE1_, List[_TYPE2_]]", TypeHintValue="{}"/
{
%TypeHeaderCode
#include <qmap.h>
%End
...
%ConvertToTypeCode
    ...
    QMultiMap<_TYPE1_, _TYPE2_> *qm = new QMultiMap<_TYPE1_, _TYPE2_>;
    ...
    while (PyDict_Next(sipPy, &i, &kobj, &vobj))
    {
        ...
        _TYPE1_ *k = reinterpret_cast<_TYPE1_ *>(sipConvertToType(kobj, sipType__TYPE1_, sipTransferObj, SIP_NOT_NONE, &kstate, sipIsErr));
        ...
        Py_ssize_t vi = PySequence_Size(vobj);

        if (vi < 0)
        {
            PyErr_Format(PyExc_TypeError,
                    "a dict value has type '%s' but a sequence is expected",
                    sipPyTypeName(Py_TYPE(vobj)));
        ...
        // We go through the sequence backwards to maintain the Qt semantics of
        // later items appearing first.
        while (vi--)
        {
            PyObject *itm_obj = PySequence_GetItem(vobj, vi);
            ...
            _TYPE2_ *itm = reinterpret_cast<_TYPE2_ *>(sipConvertToType(itm_obj, sipType__TYPE2_, sipTransferObj, SIP_NOT_NONE, &itm_state, sipIsErr));
            ...
            {
                PyErr_Format(PyExc_TypeError,
                        "a sequence element has type '%s' but '_TYPE2_' is expected",
                        sipPyTypeName(Py_TYPE(itm_obj)));
            ...
            qm->insert(*k, *itm);
            sipReleaseType(itm, sipType__TYPE2_, itm_state);
        }
