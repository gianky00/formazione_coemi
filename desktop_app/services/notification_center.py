...
from PySide6.QtWidgets import (
    ...
    QFrame, QScrollArea, QDialog, QRadioButton, QButtonGroup,
    QSizePolicy, QSpacerItem
)
from PySide6.QtCore import Qt, QTimer, Signal, Slot, QPoint, QObject

class Notification:
    def __init__(self, title, message, notification_type="info", action=None, action_label=None, category=None, priority=0):
        self.id = id(self)
        self.title = title
...
class NotificationCenter(QObject):
    ...
    def add(self, title, message, notification_type="info", action=None, action_label=None, category=None, priority=0, show_toast=True):
        notif = Notification(title, message, notification_type, action, action_label, category, priority)
        with self._lock:
    ...
    def get_all(self, include_dismissed=False):
        with self._lock:
            return list(self.notifications) if include_dismissed else [n for n in self.notifications if not n.dismissed]
    ...
    def mark_all_read(self):
        with self._lock:
            for n in self.notifications: n.read = True
        self.changed.emit()

    def clear_all(self):
        with self._lock: self.notifications.clear()
        self.changed.emit()
    ...
    def dismiss(self, nid):
        with self._lock:
            for n in self.notifications:
                if n.id == nid: n.dismissed = True; break
        self.changed.emit()
...
class NotificationBell(QWidget):
    ...
    def setup_ui(self):
        ...
        self.btn_bell.setStyleSheet("background: transparent; color: white; font-size: 20px; border: none;")
        ...
        self.badge.setStyleSheet("background-color: #EF4444; color: white; border-radius: 9px; font-size: 10px; font-weight: bold;")
...
class NotificationPanel(QFrame):
    def __init__(self, parent_bell, notification_center, controller):
        ...
        self.setStyleSheet("background-color: white; border: 1px solid #D1D5DB; border-radius: 8px;")
    ...
    def setup_ui(self):
        ...
        header.setStyleSheet("background-color: #F3F4F6; border-top-left-radius: 8px; border-top-right-radius: 8px;")
    ...
    def load_notifications(self):
        ...
        for n in notifs:
            item = QFrame()
            item.setStyleSheet(f"background-color: {'white' if n.read else '#EFF6FF'}; border-bottom: 1px solid #E5E7EB; padding: 5px;")
