...
def check_system_clock():
    ...
    if network_now:
        ...
        if time_difference > 300:
            return (
                False,
                f"L'orologio del sistema non è sincronizzato (differenza di {int(time_difference / 60)} minuti).\n"
    ...
    if not state:
        return (
            False,
            "Impossibile verificare l'ora (Nessuna connessione e nessun dato precedente).\n"
    ...
    if system_now < last_exec - timedelta(minutes=5):
        return (
            False,
            "Rilevata manomissione dell'orologio di sistema (Data antecedente all'ultima esecuzione).\n"
    ...
    if system_now > buffer_end:
        return (
            False,
            f"Periodo offline massimo ({OFFLINE_BUFFER_DAYS} giorni) scaduto.\n"
