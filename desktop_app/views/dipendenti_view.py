...
from PySide6.QtWidgets import (
    ...
    QAbstractItemView, QComboBox, QLineEdit, QListWidget
)
...
class DipendentiView(QWidget):
    ...
    def setup_ui(self):
        ...
        self.table.set_columns(["ID", "Matricola", "Cognome e Nome", "Data Nascita", "Mansione", "Reparto"])
    ...
    def filter_data(self):
        ...
        for d in self.data:
            ...
            txt = f"{display_name} {d.get('matricola','')} {d.get('categoria_reparto','')}".lower()
        ...
        mapping = ["id", "matricola", "display_name", "formatted_nascita", "mansione", "categoria_reparto"]
        self.table.load_data(filtered, mapping)

    def add_dipendente(self):
        if DipendenteDialog(self, self.controller).exec(): self.refresh_data()

    def on_double_click(self, dip):
        if dip and DipendenteDialog(self, self.controller, dip).exec(): self.refresh_data()

    def show_storico(self, dip):
        if dip: StoricoCertificatiDialog(self, self.controller, dip).exec()

    def _bulk_assign_mansione(self):
        selected = self.table.get_selected_data()
        if selected:
            ids = [str(s["id"]) for s in selected]
            if BulkAssignDialog(self, self.controller, ids, self.data).exec(): self.refresh_data()
...
class DipendenteDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.edits = {k: QLineEdit(str(self.dip_data.get(k) or "")) if self.dip_data else QLineEdit() for k in fields}
        for k, v in self.edits.items(): form.addRow(f"{k.replace('_',' ').title()}:", v)
...
class BulkAssignDialog(QDialog):
    def __init__(self, parent, controller, dip_ids, all_data):
        ...
        self.combo.addItems(sorted(set(d.get("mansione") for d in all_data if d.get("mansione"))))
        layout.addWidget(QLabel("Nuova Mansione:"))
        layout.addWidget(self.combo)
        btn_box = QDialogButtonBox(QDialogButtonBox.Ok | QDialogButtonBox.Cancel)
        btn_box.accepted.connect(self.apply); btn_box.rejected.connect(self.reject)
        layout.addWidget(btn_box)

    def apply(self):
        val = self.combo.currentText()
        for did in self.dip_ids:
            try: self.controller.api_client.update_dipendente(did, {"mansione": val})
            except: pass
        self.accept()
