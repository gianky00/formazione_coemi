...
from PySide6.QtWidgets import (
    ...
    QMenu, QMessageBox, QTabWidget, QDialog, QFormLayout, 
    QDialogButtonBox, QListWidget, QFrame, QLineEdit, QComboBox
)
...
class ValidationView(QWidget):
    ...
    def _setup_validation_tab(self):
        ...
        self.table.set_columns(["ID", "Dipendente", "Documento", "Categoria", "Rilascio", "Scadenza"])
    ...
    def _setup_orphans_tab(self):
        ...
        self.table_orphans.set_columns(["ID", "Nome Rilevato", "Documento", "Categoria", "Rilascio", "Scadenza"])
    ...
    def _refresh_orphans(self):
        mapping = ["id", "nome_dipendente_raw", "corso", "categoria", "data_rilascio", "data_scadenza"]
    ...
    def open_file(self, cert):
        ...
        else: QMessageBox.warning(self, "Errore", "File non trovato.")
    ...
    def delete_selected(self, cert=None):
        ...
        if selected and QMessageBox.question(self, "Conferma", f"Eliminare {len(selected)} certificati?") == QMessageBox.Yes:
    ...
    def _delete_orphans(self):
        ...
        if selected and QMessageBox.question(self, "Conferma", f"Eliminare {len(selected)} orfani?") == QMessageBox.Yes:
            ids = [str(s["id"]) for s in selected]
            ProgressTaskRunner(self).run(self.controller.api_client.delete_certificato, ids)
            self.refresh_data()
...
class ValidationDialog(QDialog):
    ...
    def save(self):
        ...
        try:
            ...
            requests.put(f"{self.controller.api_client.base_url}/certificati/{self.cert['id']}/valida", headers=self.controller.api_client._get_headers())
            self.accept()
        except Exception as e: QMessageBox.critical(self, "Errore", str(e))
...
class AssignOrphanDialog(QDialog):
    ...
    def setup_ui(self):
        layout = QVBoxLayout(self)
        self.search = QLineEdit(); self.search.setPlaceholderText("Cerca dipendente...")
    ...
    def refresh_list(self):
        ...
        self.list.clear()
        self.filtered = [d for d in self.dipendenti if q in f"{d.get('cognome','')} {d.get('nome','')}".lower()]
        for d in self.filtered: self.list.addItem(f"{d.get('cognome','')} {d.get('nome','')}")
    ...
    def assign(self):
        ...
        if idx >= 0:
            ...
            for cid in self.cert_ids:
                try: self.controller.api_client.update_certificato(cid, {"dipendente_id": dip["id"], "nome": f"{dip['cognome']} {dip['nome']}"})
                except: pass
            self.accept()
