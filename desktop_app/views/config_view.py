...
from PySide6.QtWidgets import (
    ...
    QLineEdit, QCheckBox, QTabWidget, QScrollArea, QFrame, 
    ...
    def refresh_users(self):
        try:
            ...
            for u in users:
                ...
                self.table.setItem(row, 2, QTableWidgetItem("Admin" if u.get("is_admin") else "User"))
                self.table.setItem(row, 3, QTableWidgetItem(u.get("last_login", "")))
        except: pass

    def add_user(self):
        if UserDialog(self, self.controller).exec(): self.refresh_users()
