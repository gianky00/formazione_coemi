...
def _run_pyarmor():
    ...
    cmd_pyarmor = [
        ...
        os.path.join(ROOT_DIR, "desktop_app"),
...
def _prepare_assets():
    ...
    if os.path.exists(os.path.join(ROOT_DIR, FILE_REQUIREMENTS)):
        shutil.copy(
            os.path.join(ROOT_DIR, FILE_REQUIREMENTS), os.path.join(OBF_DIR, FILE_REQUIREMENTS)
        )
...
def _prepare_pyinstaller_cmd(runtime_dir):
    ...
    cmd = [
        ...
        "--noconfirm",
        "--icon",
        os.path.join(OBF_DIR, "desktop_app", "icons", "icon.ico"),
        "--distpath",
        DIST_DIR,
        "--workpath",
...
def build():
    ...
    try:
        ...
        iss_path = os.path.abspath(
            os.path.join(ROOT_DIR, "admin", "crea_setup", "setup_script.iss")
        )
