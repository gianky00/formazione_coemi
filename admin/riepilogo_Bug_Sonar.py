...
stats = {"api_calls": 0, "api_errors": 0, "retries": 0, "start_time": None, "end_time": None}
...
def clean_html(text, preserve_code_blocks=True):
    ...
    if preserve_code_blocks:
        ...
        for i, block in enumerate(code_blocks):
            text = re.sub(
                r"<pre[^>]*>" + re.escape(block) + r"</pre>", f"__CODE_BLOCK_{i}__", text, count=1
            )
...
def get_severity_emoji(severity):
    return {"BLOCKER": "🔴", "CRITICAL": "🔴", "MAJOR": "🟡", "MINOR": "🟢", "INFO": "⚪"}.get(
        severity, "⚪"
    )
...
def parse_junit_xml():
    ...
    try:
        ...
        summary["passed"] = (
            summary["total"] - summary["failures"] - summary["errors"] - summary["skipped"]
        )
...
def get_source_lines(component, start_line, end_line, context=3):
    ...
    try:
        data = api_request(
            API_SOURCES,
            {"key": component, "from": max(1, start_line - context), "to": end_line + context},
        )
        if data:
            lines = []
...
def fetch_security_hotspots():
    ...
    while True:
        data = api_request(
            API_HOTSPOTS,
            {"projectKey": PROJECT_KEY, "status": HOTSPOT_STATUSES, "ps": PAGE_SIZE, "p": page},
        )
    ...
    for i, hotspot in enumerate(hotspots):
        try:
            ...
            if data:
                ...
                hotspot["vulnerability_description"] = clean_html(
                    rule.get("vulnerabilityDescription", "")
                )
...
def fetch_source_code(issues):
    ...
    for i, issue in enumerate(target):
        ...
        if line and component:
            ...
            issue["source_code"] = get_source_lines(
                component, tr.get("startLine", line), tr.get("endLine", line), 2
            )
        if (i + 1) % 50 == 0:
...
def _generate_main_prompt(
    total_issues, total_hotspots, total_test_failures, total_effort, issues_count
):
...
def generate_dynamic_prompts(
    issues, analysis, issues_count, hotspots, hotspot_analysis, test_analysis
):
    ...
    prompts["main"] = _generate_main_prompt(
        total_issues, total_hotspots, total_test_failures, total_effort, issues_count
    )

    if total_test_failures > 0:
        prompts["test_failures"] = _generate_test_failures_prompt(
            total_test_failures, test_analysis
        )
...
def _generate_quality_gate_section(md):
    ...
    if conditions:
        ...
        for c in conditions:
            ...
            md.append(
                f"| {c.get('metricKey', '')} | {c.get('actualValue', 'N/A')} | {c.get('errorThreshold', 'N/A')} | {c_emoji} |"
            )
        md.append("")
...
def _generate_test_results_section(md, junit_summary, test_analysis):
    ...
    if junit_summary:
        ...
        md.extend(
            [
                "1. Esegui `pytest --junitxml=junit.xml`",
        ...
        md.extend(
            [
                "⚠️ **Nessun dato test disponibile**",
                "",
                "Per abilitare:",
...
def _generate_coverage_section(md):
    ...
    md.extend(
        [
            f"| Linee da coprire | {coverage_metrics.get('lines_to_cover', 'N/A')} |",
...
def _generate_hotspots_section(md, hotspots, hotspot_analysis):
    ...
    md.extend(
        [
            "## 🔥 Security Hotspots",
            "",
            f"**Totale:** {len(hotspots)} da revieware",
            "",
...
def _generate_top_rules_section(md, analysis):
    ...
    if not analysis["top_rules"]:
        return
    md.extend(
        ["## 🏆 Top 10 Regole Violate", "", "| # | Regola | Count |", "|---|--------|-------|"]
    )
...
def generate_summary(
    ...
    # Prompts
    prompts = generate_dynamic_prompts(
        issues, analysis, issues_count, hotspots, hotspot_analysis, test_analysis
    )
...
def generate_quality_specific_file(issues, quality, output_file):
    ...
    md = [
        f"# {emoji} {quality} Issues",
        "",
        f"**Totale:** {len(qi)}",
        f"**Tempo:** {format_duration(effort)}",
        "",
        "---",
        "",
    ]
...
def generate_compact(issues, output_file):
    ...
    counts = {
        q: sum(1 for i in issues if i.get("software_quality") == q) for q in SOFTWARE_QUALITIES
    }
...
def _print_junit_summary(junit_summary):
    if junit_summary:
        ...
        print(
            f"      Tests: {junit_summary['total']}, Failed: {junit_summary['failures']}, Errors: {junit_summary['errors']}"
        )
    else:
...
def _print_generated_files(
    ...
    if test_failures_details:
        print(
            f"   🧪 JULES_TEST_FAILURES_{timestamp}.md - {len(test_failures_details)} test falliti"
        )
    if all_hotspots:
...
def _main_fetch_metrics():
    ...
    if test_metrics:
        print(
            f"   ✓ SonarCloud: {test_metrics.get('tests', 0)} test, {test_metrics.get('test_failures', 0)} falliti"
        )
    if coverage_metrics:
...
def main():
    ...
    print(
        f"   ✓ {analysis['total']} issues, {len(all_hotspots)} hotspots, {len(test_failures_details)} test falliti"
    )
