...
from app.core.path_resolver import (
    get_base_path, get_asset_path, get_user_data_path, 
    get_license_path, get_database_path, get_logs_path
)
...
def test_base_path_contains_project_structure():
    ...
    print(f"   Path: {base}")
    print(f"   ✅ PASS: Struttura progetto trovata")
    return True
...
def test_guide_html_loading():
    ...
    if guide_html.exists():
        ...
        print(f"   ❌ FAIL: React SPA non trovata")
        print(f"   → Esegui: cd guide_frontend && npm run build")
        return False
    ...
    try:
        ...
        if license_file and os.path.exists(license_file):
            ...
            if config_file.exists():
                ...
                print(f"   ✅ PASS (via path_resolver)")
                return True
            else:
                print(f"   License dir: {license_dir}")
                print(f"   ❌ FAIL: config.dat non trovato")
                return False
...
def test_database_path():
    ...
    try:
        ...
        if str(user_data) in str(db_path):
            print(f"   Location: User data directory ✓")
            print(f"   ✅ PASS")
            return True
        else:
            print(f"   ⚠️ WARNING: DB path not in user data")
...
def test_user_data_path_writable():
    ...
    try:
        ...
        if test_file.exists():
            ...
            print(f"   ✅ PASS: Directory scrivibile")
            return True
        else:
            print(f"   ❌ FAIL: Write verification failed")
            return False
...
def test_logs_path():
    ...
    if logs_path.exists() and logs_path.is_dir():
        ...
        print(f"   Path: {logs_path}")
        print(f"   ❌ FAIL: Directory non trovata")
        return False
...
def test_icon_assets():
    ...
    if found_any:
        print(f"   ✅ PASS: Almeno un'icona trovata")
        return True
    else:
        print(f"   ❌ FAIL: Nessuna icona trovata")
        return False
...
def test_path_resolver_consistency():
    ...
    if str(user_data) not in str(db_path):
        print(f"   ⚠️ WARNING: db_path non in user_data")
    
    if str(user_data) not in str(logs_path):
        print(f"   ⚠️ WARNING: logs_path non in user_data")
