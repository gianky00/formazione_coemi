...
from app.db.models import Certificato, Corso, User
...
def migrate_schema(db: Session):
    ...
    try:
        ...
        db.execute(
            text(
                "UPDATE audit_logs SET category = 'CERTIFICATE' WHERE category = 'GENERAL' AND action LIKE 'CERTIFICATE_%'"
            )
        )
        db.execute(
            text(
                "UPDATE audit_logs SET category = 'AUTH' WHERE category = 'GENERAL' AND (action = 'LOGIN' OR action = 'LOGOUT')"
            )
        )
        db.execute(
            text(
                "UPDATE audit_logs SET category = 'USER_MGMT' WHERE category = 'GENERAL' AND (action LIKE 'USER_%' OR action = 'PASSWORD_CHANGE')"
            )
        )
        db.execute(
            text(
                "UPDATE audit_logs SET category = 'DATA' WHERE category = 'GENERAL' AND action = 'DIPENDENTI_IMPORT'"
            )
        )
        db.execute(
            text(
                "UPDATE audit_logs SET category = 'SYSTEM' WHERE category = 'GENERAL' AND action = 'SYSTEM_ALERT'"
            )
        )
        db.execute(
            text(
                "UPDATE audit_logs SET category = 'CONFIG' WHERE category = 'GENERAL' AND action = 'CONFIG_UPDATE'"
            )
        )
        db.commit()
...
def cleanup_deprecated_data(db: Session):
    ...
    try:
        ...
        trash_dir = (
            os.path.join(database_path, "DOCUMENTI DIPENDENTI", "CESTINO")
            if database_path
            else None
        )
        ...
        if certs_to_delete:
            print(
                f"Cleanup: Deleted {len(certs_to_delete)} certificates for deprecated category '{deprecated_category}'."
            )

        deleted_courses = (
            db.query(Corso).filter(Corso.id.in_(course_ids)).delete(synchronize_session=False)
        )
...
def _move_deprecated_file(cert, database_path, trash_dir):
    try:
        cert_data = {
            ...
            "data_scadenza": cert.data_scadenza_calcolata.strftime("%d/%m/%Y")
        ...
        if database_path:
            ...
            if file_path and os.path.exists(file_path):
                ...
                dest = os.path.join(
                    trash_dir, f"{os.path.splitext(filename)[0]}_deprecated_{timestamp}.pdf"
                )
...
def seed_database(db: Session = None):
    ...
    try:
        ...
        corsi = [
            ...
            {
                "nome_corso": "DIRETTIVA SEVESO",
                "validita_mesi": 60,
                "categoria_corso": "DIRETTiva SEVESO",
            },
            {
                "nome_corso": "DIRIGENTI E FORMATORI",
                "validita_mesi": 60,
                "categoria_corso": "DIRIGENTI E FORMATORI",
            },
            {
                "nome_corso": "GRU A TORRE E PONTE",
                "validita_mesi": 60,
                "categoria_corso": "GRU A TORRE E PONTE",
            },
            {"nome_corso": "H2S", "validita_mesi": 60, "categoria_corso": "H2S"},
            {"nome_corso": "IMBRACATORE", "validita_mesi": 60, "categoria_corso": "IMBRACATORE"},
            {
                "nome_corso": "FORMAZIONE GENERICA ART.37",
                "validita_mesi": 60,
                "categoria_corso": "FORMAZIONE GENERICA ART.37",
            },
            {"nome_corso": "PREPOSTO", "validita_mesi": 24, "categoria_corso": "PREPOSTO"},
            {
                "nome_corso": "GRU SU AUTOCARRO",
                "validita_mesi": 60,
                "categoria_corso": "GRU SU AUTOCARRO",
            },
            {"nome_corso": "PLE", "validita_mesi": 60, "categoria_corso": "PLE"},
            {
                "nome_corso": "L5 PES PAV PEI C CANTIERE",
                "validita_mesi": 60,
                "categoria_corso": "L5 PES PAV PEI C CANTIERE",
            },
            {
                "nome_corso": "LAVORI IN QUOTA",
                "validita_mesi": 60,
                "categoria_corso": "LAVORI IN QUOTA",
            },
            {
                "nome_corso": "MACCHINE OPERATRICI",
                "validita_mesi": 60,
                "categoria_corso": "MACCHINE OPERATRICI",
            },
            {
                "nome_corso": "MANITOU P.ROTATIVE",
                "validita_mesi": 60,
                "categoria_corso": "MANITOU P.ROTATIVE",
            },
            {
                "nome_corso": "MULETTO CARRELISTI",
                "validita_mesi": 60,
                "categoria_corso": "MULETTO CARRELISTI",
            },
            {
                "nome_corso": "SOPRAVVIVENZA E SALVATAGGIO IN MARE",
                "validita_mesi": 60,
                "categoria_corso": "SOPRAVVIVENZA E SALVATAGGIO IN MARE",
            },
            {
                "nome_corso": "SPAZI CONFINATI DPI III E AUTORESPIRATORI",
                "validita_mesi": 60,
                "categoria_corso": "SPAZI CONFINATI DPI III E AUTORESPIRATORI",
            },
            ...
            {
                "nome_corso": "CARTA DI IDENTITA",
                "validita_mesi": 0,
                "categoria_corso": "CARTA DI IDENTITA",
            },
            {
                "nome_corso": "MODULO RECESSO RAPPORTO DI LAVORO",
                "validita_mesi": 0,
                "categoria_corso": "MODULO RECESSO RAPPORTO DI LAVORO",
            },
