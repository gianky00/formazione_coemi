...
class CertificateService:
    ...
    def get_all(self, validated: Optional[bool] = None) -> List[Certificato]:
    ...
    def get_by_id(self, cert_id: int) -> Optional[Certificato]:
    ...
    def create(self, data: CertificatoCreazioneSchema) -> Certificato:
        ...
        new_cert = Certificato(
            ...
            data_scadenza_calcolata=parse_date_flexible(data.data_scadenza) if data.data_scadenza else None,
    ...
    def build_schema(self, cert: Certificato, status_map: dict = None) -> CertificatoSchema:
        """Helper per trasformare il modello in schema."""
        status = status_map.get(int(cert.id), "attivo") if status_map else certificate_logic.get_certificate_status(self.db, cert)
        
        if cert.dipendente:
            nome = f"{cert.dipendente.cognome} {cert.dipendente.nome}"
            matr = cert.dipendente.matricola
            dob = cert.dipendente.data_nascita.strftime(DATE_FORMAT_DMY) if cert.dipendente.data_nascita else None
        else:
        ...
        return CertificatoSchema(
            ...
            assegnazione_fallita_ragione=None if cert.dipendente else "Mancata associazione anagrafica"
        )

    def get_orphan_data(self, cert: Certificato) -> dict:
        return {
    ...
    def sync_file_system(self, cert: Certificato, old_data: dict) -> None:
    ...
    def _get_or_create_course(self, cat: str, name: str) -> Corso:
        course = self.db.query(Corso).filter(Corso.nome_corso.ilike(name), Corso.categoria_corso.ilike(cat)).first()
        if not course:
            course = Corso(nome_corso=name, categoria_corso=cat, validita_mesi=0)
            self.db.add(course); self.db.commit(); self.db.refresh(course)
        return course

    def _check_duplicate(self, course_id, rilascio, dip_id, nome) -> bool:
        dt = parse_date_flexible(str(rilascio))
        query = self.db.query(Certificato).filter(Certificato.corso_id == course_id, Certificato.data_rilascio == dt)
        if dip_id: query = query.filter(Certificato.dipendente_id == dip_id)
        else: query = query.filter(Certificato.nome_dipendente_raw == nome)
        return query.first() is not None

    def _archive_obsolete(self, cert: Certificato):
        if not cert.dipendente_id or not cert.corso: return
        older = self.db.query(Certificato).join(Corso).filter(
        ...
        for o in older: sync_service.archive_certificate_file(self.db, o)

    def _try_link_file(self, cert: Certificato, data: Any):
        db_path = settings.DOCUMENTS_FOLDER or str(get_user_data_dir())
        f_data = {"nome": data.nome, "matricola": cert.dipendente.matricola if cert.dipendente else None, "categoria": data.categoria, "data_scadenza": cert.data_scadenza_calcolata.strftime(DATE_FORMAT_DMY) if cert.data_scadenza_calcolata else None}
        found = find_document(db_path, f_data)
        if found: cert.file_path = found; self.db.commit()

    def _update_fields(self, cert: Certificato, data: dict):
        ...
        if "data_rilascio" in data: cert.data_rilascio = parse_date_flexible(data["data_rilascio"])
        if "data_scadenza" in data: cert.data_scadenza_manuale = parse_date_flexible(data["data_scadenza"])
