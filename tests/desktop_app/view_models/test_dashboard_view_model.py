...
def mock_api_data():
    return [
        {'id': 1, 'nome': 'Mario Rossi', 'categoria': 'Formazione', 'stato_certificato': 'attivo'},
        {'id': 2, 'nome': 'Luigi Bianchi', 'categoria': 'Sicurezza', 'stato_certificato': 'scaduto'},
        {'id': 3, 'nome': 'Mario Rossi', 'categoria': 'Sicurezza', 'stato_certificato': 'attivo'},
    ]
...
def test_load_data_success(mock_get, mock_api_data):
    ...
    assert list(vm.filtered_data['nome']) == ['Mario Rossi', 'Luigi Bianchi', 'Mario Rossi']
...
def test_filter_data(mock_get, mock_api_data):
    ...
    # Filter by employee
    vm.filter_data('Mario Rossi', 'Tutti', 'Tutti')
    assert vm.filtered_data.shape[0] == 2
    assert all(vm.filtered_data['nome'] == 'Mario Rossi')
    ...
    # Combined filter
    vm.filter_data('Mario Rossi', 'Sicurezza', 'Tutti')
    assert vm.filtered_data.shape[0] == 1
    assert vm.filtered_data.iloc[0]['nome'] == 'Mario Rossi'
...
def test_get_filter_options(mock_get, mock_api_data):
    ...
    assert options['dipendenti'] == ['Luigi Bianchi', 'Mario Rossi']
