...
def test_voice_service_speak():
    ...
    with patch.object(voice_service, 'TTSWorker') as MockWorkerClass:
        with patch.object(voice_service, 'settings') as mock_settings:
            ...
            # Assert called with correct args - only text now
            MockWorkerClass.assert_called_with(text="Test")
...
def test_tts_worker_run():
    """
    Test the TTSWorker run method mocking gTTS.
    """
    worker = TTSWorker("text")

    # Use patch.object on the imported module instance to ensure correct patching
    # regardless of sys.modules state (which might be tricky due to the patch.dict above)
    with patch.object(voice_service, 'gTTS') as mock_gtts_class:
        mock_tts_instance = MagicMock()
        mock_gtts_class.return_value = mock_tts_instance

        # Patch tempfile.mkstemp via the module import reference
        with patch.object(voice_service.tempfile, 'mkstemp', return_value=(1, "/tmp/test_gtts.mp3")):
            # Patch os.close via the module import reference
            with patch.object(voice_service.os, 'close'):

                worker.run()

                # Verify gTTS init
                mock_gtts_class.assert_called_with(
                    text="text",
                    lang='it',
                    tld='com',
                    slow=False
                ...
                # Verify save call
                mock_tts_instance.save.assert_called_with("/tmp/test_gtts.mp3")
