...
def test_upload_pdf_visita_medica(test_client: TestClient, db_session: Session, mocker):
    ...
    seed_master_courses(db_session)
    mocker.patch(
        "app.services.ai_extraction.extract_entities_with_ai",
        return_value={
            "nome": "Mario Rossi",
            "corso": "Giudizio di idoneità",
...
def test_update_data_scadenza_variations(
    test_client: TestClient, db_session: Session, scadenza_in, db_out, resp_out
):
