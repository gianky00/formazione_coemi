...
def test_update_user_duplicate_username(test_client: TestClient, db_session: Session):
    ...
    user1 = User(
        username="user1", hashed_password=security.get_password_hash("password"), is_admin=False
    )
    user2 = User(
        username="user2", hashed_password=security.get_password_hash("password"), is_admin=False
    )
