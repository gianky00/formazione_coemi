...
def test_immutable_settings_are_hardcoded(monkeypatch, tmp_path):
    ...
    assert (
        settings.SECRET_KEY
        == "a_very_strong_and_long_secret_key_that_is_not_easily_guessable_and_is_unique_to_this_application"
    )
...
def test_migration_of_legacy_gemini_key(settings_file: Path, monkeypatch):
    ...
    """
    legacy_settings = {"GEMINI_API_KEY": "legacy_key_123", "SMTP_HOST": "old.smtp.com"}
    with open(settings_file, "w") as f:
...
def test_github_token_is_revealed_at_runtime(monkeypatch, tmp_path):
    ...
    assert expected_token == settings.LICENSE_GITHUB_TOKEN
    # Verify the raw, internal value is still obfuscated
    assert expected_token != settings._OBFUSCATED_GITHUB_TOKEN
...
def test_gemini_api_key_is_revealed_at_runtime(settings_file: Path, monkeypatch):
    ...
    assert plain_key == settings.GEMINI_API_KEY_ANALYSIS
